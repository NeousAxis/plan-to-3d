...
> - 🟡 Étape 5A (lightmap embedded) — **`bake_lightmap.py` + `bake_lightmap_blender.py` : pipeline lightmap UV2 dans le GLB**. Le bake passe (UV unwrap, 1024×1024 par mesh, GI cuit, exporté en `model_lit.glb`). Le viewer promote `emissiveMap` UV2 → `material.lightMap` Three.js natif. **Le pipeline fonctionne mais l'effet visuel reste subtil** : la lightmap multiplie la base color, et avec une scène déjà bien éclairée elle satue. Demanderait encore du calibrage (réduire encore l'éclairage temps réel, tweaker `lightMapIntensity`, possiblement basculer en MeshLambert pour ces meshes). Pour le moment, le **rendu still Cycles** reste la voie qui donne le vrai look photoréaliste.
