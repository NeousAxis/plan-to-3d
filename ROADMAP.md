...
> - ✅ Étape 5A — **`bake.py` + `bake_blender.py` : pipeline Blender Cycles headless**. Author-side GI photoréaliste (flaques de lumière au sol, indirect bounces, soft shadows, sky Hosek-Wilkie). Auto-config GPU Metal sur M3. Furniture lights → Cycles lights. Bake d'un lobby = 64-256 samples = 1-5 min sur M3 Max. Sortie `render.png` partageable par tout le monde (« pour tout le monde » préservé : calcul lourd côté créateur, image légère pour spectateurs). **Le viewer 3D temps réel reste inchangé.** Cuit dans le GLB (UV2 + lightMap) reste à faire pour avoir le GI interactif live, mais le still photoréaliste fonctionne.
